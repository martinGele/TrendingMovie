package com.martin.trendingmovie.features.detail
...
import com.martin.trendingmovie.convertToFavoriteTable
import com.martin.trendingmovie.convertToTrendingTvShowTable
...
class DetailViewModel @Inject constructor(
    ...
    private val _similarTvShows: MutableStateFlow<PagingData<TrendingTvShowsTable>> = MutableStateFlow(PagingData.empty())
    val similarTvShows: StateFlow<PagingData<TrendingTvShowsTable>> = _similarTvShows

    fun getSimilarTvShows(tvShowId: Int) {
        viewModelScope.launch {
            repository.getSimilarTvShows(tvShowId = tvShowId).cachedIn(viewModelScope).collect { movies ->
    ...
    private fun deleteFavoriteTvShow(tvShowId: Int) = viewModelScope.launch { repository.deleteFavoriteTvShow(tvShowId = tvShowId) }
